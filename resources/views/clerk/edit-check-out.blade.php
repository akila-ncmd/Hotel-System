...
<div class="container">
    ...
    <form method="POST" action="{{ route('clerk.update-check-out', $reservation->id) }}" id="editCheckOutForm">
        ...
        <div class="mb-3">
            ...
            <input type="text" id="total-amount" class="form-control" value="@money($baseCost + ($billing->restaurant_charges ?? 0) + ($billing->room_service_charges ?? 0) + ($billing->laundry_charges ?? 0) + ($billing->telephone_charges ?? 0) + ($billing->club_facility_charges ?? 0))" disabled>
        </div>
        <div class="mb-3">
