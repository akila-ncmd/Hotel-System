...
<div class="container">
    ...
    @endif
    <form method="POST" action="{{ route('clerk.update-check-out', $reservation->id) }}" id="editCheckOutForm">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" id="total-amount" class="form-control" value="${{ number_format($baseCost + ($billing->restaurant_charges ?? 0) + ($billing->room_service_charges ?? 0) + ($billing->laundry_charges ?? 0) + ($billing->telephone_charges ?? 0) + ($billing->club_facility_charges ?? 0), 0) }}" disabled>
        </div>
        <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('editCheckOutForm').addEventListener('submit', (e) => {
        let isValid = true;
        chargeInputs.forEach(input => {
